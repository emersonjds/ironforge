...
- Borda compartilhada contínua (`border border-forest-500/40 rounded-xl`) envolvendo os dois blocos de exercício, com um traço vertical forest (`w-0.5 bg-forest-500`) conectando os dois headers.
- Label `SUPERSET A/B` aparece acima do par (uppercase, `text-2xs font-semibold tracking-widest text-forest-400`).
...
│  │  2  │ 100×8    │ 100  │  8   │ ● │  │
│  │  3  │ 100×7    │[100] │ [_]  │ ○ │  │  ← ATIVO (highlight forest)
│  │  4  │ 100×6    │ 100  │  -   │ ○ │  │
│  └───────────────────────────────────┘  │
│                                         │
│  + Adicionar série                      │
...
module.exports = {
  ...
  theme: {
    extend: {
      colors: {
        // BACKGROUND / SURFACE — Forest Minimalist, light-first; profundidade por contorno
        bg: {
          DEFAULT: "#F9FAFB", // tela / "wells"
          raised:  "#FFFFFF", // cards
          sunken:  "#F3F4F5", // wells profundos
          overlay: "rgba(25,28,29,0.40)", // scrim de modais
        },
        surface: { // neutros (50 claro → 950 escuro)
          50: "#FFFFFF", 100: "#F9FAFB", 200: "#F3F4F5", 300: "#EDEEEF",
          400: "#E1E3E4", 500: "#C1C8C2", 600: "#A8AFA9", 700: "#717973",
          800: "#414844", 900: "#191C1D", 950: "#0B0F0E",
        },
        forest: { // marca (verde-floresta)
          50: "#EBF5EF", 100: "#D8F3DC", 200: "#B7E4C7", 300: "#86AF99",
          400: "#2D6A4F", 500: "#1B4332", 600: "#143728", 700: "#0E2B1F",
          800: "#08231A", 900: "#012D1D", 950: "#01160E",
        },
        ...
        text: {
          primary: "#191C1D", secondary: "#414844", tertiary: "#717973",
          disabled: "#A8AFA9", inverse: "#FFFFFF", accent: "#1B4332",
        },
        border: {
          subtle: "#F0F1F2", DEFAULT: "#E5E7EB", strong: "#C1C8C2", accent: "#1B4332",
        },
      },
      fontFamily: {
...
| **success** | Toast `bg-success-muted` border-l verde 240ms. |
...
├─────────────────────────────────────────┤
│  ┌─────────────────────────────────────┐ │
│  │         = ANTERIOR  100×8           │ │  ← primário 56dp fullwidth, bg-forest-500/20
│  │              border-forest-500       │ │    border-forest-500; 1 toque confirma tudo
│  └─────────────────────────────────────┘ │
├─────────────────────────────────────────┤
│   7    8    9         [+2.5]  [+5]      │
│   4    5    6         [-2.5]  [-5]      │
...
- Progress: `Progress value={50} variant="forest"` (height 3dp)
...
- **Supersets na tabela de sets:** dois blocos de exercício com `border-l-2 border-forest-500` compartilhado como traço vertical conectando os dois headers. Header acima do par alterna: `SUPERSET 1/2` durante exercício A, `SUPERSET 2/2` durante exercício B — em `text-2xs font-semibold tracking-widest text-forest-400`. Rest timer: estado `disabled` (cinza, inativo) enquanto no primeiro exercício do par; ativa apenas após confirmar o último set do segundo. O visual do timer no bottom CTA reflete isso: área do timer aparece em `surface-400` com label `APÓS O PAR` durante exercício A.

**Estados**
| Estado | Comportamento |
|---|---|
| **default** | Set ativo destacado com ring `border-forest-500`. |
| **logging** | Row pisca `bg-success-muted` 240ms, scale 1→1.02→1, haptic `impactMedium`, próxima row vira ativa, rest timer dispara. |
| **PR detectado** | `PRCelebrationModal` overlay 560ms, confete sóbrio (8 partículas verdes), haptic `notificationSuccess`. |
...
- Card ATIVO: `Box bg="bg-raised" border border-forest-500/30 rounded="xl" p="6"`
...
| Tap "Ativar" | row `bg 0→forest-muted→bg-raised` flash | 360ms | iron | impactMedium |
...
- Header: `HStack justifyContent="space-between"` com ícone checkmark `forest-500` + nome da sessão `Heading size="lg"` + `Pressable` compartilhar (abre share sheet nativa)
...
- PR badge: `Box bg="forest-500/10" border border-forest-500/40 rounded-lg p="4"` com texto `font-semibold` + e1RM estimado como `text-secondary`
...
| Primeiro treino deste plano | `forest-400` | ★ | `primeira referência` |
...
| **Primeira sessão do plano** | Seção delta substituída por "Primeira sessão registrada. Próxima vez vamos comparar aqui." + todos os exercícios com ícone ★ `forest-400`. |
...
**O que:** CTA do logger tem dois estados visuais coexistindo. Antes de logar: botão único `✓ COMPLETAR SÉRIE`. Após confirmar: split button `[⏱ 02:00] [✓ COMPLETAR SÉRIE]` onde esquerda mostra timer ativo, direita está pré-armada pra próxima série. O botão direita só ativa quando descanso atinge 80% (transição `disabled→enabled` com realce verde).
...
**Spec:** `Reanimated layout animation` com `LinearTransition` 240ms easing `anvil`. Disabled: `opacity 0.4`, `bg surface-400`. Ativo: `bg forest-500` com `boxShadow: glow`.

### 6.3 "Force unlock" no botão de encerrar treino — tap-and-hold
**O que:** o X no header do logger é press-and-hold 600ms com ring de progresso forest preenchendo. Tap simples mostra tooltip "Segure para sair" — nada mais.
...
**O padrão:** no `KeypadSheet` do logger, o botão "= anterior" ocupa o topo em 56dp × fullwidth com `bg-forest-500/20 border border-forest-500`. O teclado numérico fica abaixo, de acesso secundário.
...
**Spec definitiva:** `Pressable` 56dp height, `w-full`, `bg-forest-500/20`, `border border-forest-500`, `rounded-lg`, label `= ANTERIOR · [peso×reps da última sessão]` em `font-semibold text-base text-forest-300`. Ao tap: preenche os três campos do keypad (kg, reps, rir) com os valores da última série correspondente, avança para campo RIR para confirmação ou toque em `✓ CONFIRMAR`.
...
- Light-first (Forest Minimalist): tema claro é o padrão. Dark mode é fase futura.
- Tipografia: **Inter** para display e corpo. `font-mono` reservado a dados numéricos tabulares (carga/reps/timer).
